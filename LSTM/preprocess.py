...
def preprocess(mod, in_dir, out_dir, num_workers):
    ...
    for m in metadata:
        if m[3] not in lf0s:
            ...
            lf0s[m[3]].extend(m[4])
    with io.open(os.path.join(out_dir, 'norm.txt'), 'w', encoding='utf-8') as f:
...
def write_metadata(metadata, out_dir):
    with io.open(os.path.join(out_dir, 'train.txt'), 'w', encoding='utf-8') as f:
        for m in metadata:
            f.write(unicode('|'.join([str(x) for x in m[:-1]]) + '\n'))
